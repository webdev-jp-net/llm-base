...
- **すべてのコミット**で、コミットメッセージ末尾にIssue番号（`#[番号]`）を必ず付与する。唯一の例外は、releaseスキルが作るバージョンアップのコミット（Issueに紐づかないため付与しない）。
- **AI（Claude Code）の絶対遵守事項**:
  - コミット実行前に必ずIssue番号の有無をチェックする
  - コミット後に必ず `git log --oneline -1` でIssue番号を検証する（例外に当たるコミットでは、Issue番号が付いていないことを確認する）
...
3. **ブランチ作成**: ローカルにだけ作成する
4. **初回コミット**: 作業内容をコミット
5. **リモートへ反映**: PR作成時に `git push -u origin [ブランチ名]`
